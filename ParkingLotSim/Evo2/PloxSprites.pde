class PloxSprite extends Sprite{
  float lifetime;
  float speed, radius, nextX, nextY;
  double moveCounter, variability;
  ArrayList<PixSprite> foodList;
  
  int speedPenalty = 4;
  
  int setLife; // for all starting conditions that i want to save
  int direction = 0, moveTime = 2;  // seperate because it isn't declared with constructor
  //final int speed = ; //spaces moved per tick
  
  
  PloxSprite(int life, float sp, float x, float y, double v, float r, ArrayList<PixSprite> food){ //constuctor/////////////////////////////
    super(x, y);
    speed = sp;
    lifetime = life;
    setLife = life;
  ...
  void spriteCycle(){ ////////////////////Cycle////////////////////
    ...
    if (lifetime > 0){
      fill(50*speed, 15*speed, 15*speed); //red = speed// //green = life// //blue = radius//
  ...
  boolean hitFoodSprite(float x, float y, ArrayList<PixSprite> sprites){
  ...
  int whichSprite(float x, float y, ArrayList<PixSprite> sprites){
    int i = 0;
    for(Sprite s : sprites){
      float yCoordBottom = y + size, S_yCoordBottom = s.yCoord + s.size; 
      float xCoordRight = x + size, S_xCoordRight = s.xCoord + s.size; 
  ...
  void reproduce(){
    ...
    addNewPlox(setLife + (int)random(-20,20), speed + random(-0.2,0.2), xCoord, yCoord, variability + random(-0.1 , 0.1), radius + random(-1, 1), foodList);
    
  }
  
  int findFood(ArrayList<PixSprite> arr){
  ...
  void moveToClosest(int i, ArrayList<PixSprite> arr){
